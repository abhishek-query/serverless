...
class Demisto:
    ...
    def __init__(self, context):
        ...
        self.is_debug = False
        self._args = dict(self.callingContext.get('args', {}))
    ...
    def parentEntry(self):
        return self.callingContext['context']['ParentEntry']

    def context(self):
        return self.callingContext['context']['ExecutionContext']

    def integrationInstance(self):
        self.raise_exception_if_not_implemented(INTEGRATION, 'integrationInstance')
        return self.callingContext['context']['IntegrationInstance']
    ...
    def params(self):
        if self.is_integration:
            return self.callingContext.get('params', {})
        else:
            return {}

    def command(self):
        self.raise_exception_if_not_implemented(INTEGRATION, 'command')
        return self.callingContext.get('command', '')
...
def demisto_print(*args):
    global demisto
    output = StringIO()
    builtins.print(*args, file=output)

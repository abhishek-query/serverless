...
def _find_relevant_module(line):
    ...
    relevant_module = ''
    for module, info in list(_MODULES_LINE_MAPPING.items()):
...
def fix_traceback_line_numbers(trace_str):
    ...
    for number in re.findall(r'line (\d+)', trace_str):
        ...
        if module:
            ...
            for module_info in list(_MODULES_LINE_MAPPING.values()):
...
if IS_PY3:
    ...
    STRING_TYPES = (str, str)  # type: ignore # noqa: F821
...
class FeedIndicatorType(object):
    ...
    def list_all_supported_indicators():
        indicator_types = []
        for key, val in list(vars(FeedIndicatorType).items()):
...
class JsonTransformer:
    ...
    def json_to_str(self, json_input, is_pretty=True):
        ...
        if self.flatten:
            ...
            return '\n'.join(
                ['{key}: {val}'.format(key=k, val=flattenCell(v, is_pretty)) for k, v in list(json_input.items())])  # for BC
...
def tableToMarkdown(name, t, headers=None, headerTransform=None, removeNull=False, metadata=None, url_keys=None,
    ...
    if not headers and isinstance(t, dict) and len(list(t.keys())) == 1:
...
def fileResult(filename, data, file_type=None):
    ...
    # pylint: disable=undefined-variable
    if (IS_PY3 and isinstance(data, str)) or (not IS_PY3 and isinstance(data, str)):  # type: ignore # noqa: F821
...
def camelize_string(src_str, delim='_', upper_camel=True):
    ...
    camelize_without_first_char = ''.join([x.title() for x in components[1:]])
    if upper_camel:
...
class EntityRelationship:
    ...
    class Relationships(object):
        ...
        def is_valid(_type):
            ...
            :rtype: ``bool``
            """
            return _type in list(EntityRelationship.Relationships.RELATIONSHIPS_NAMES.keys())
    ...
    def to_context(self):
        ...
        if indicators:
            for indicator in indicators:
                ...
                for key, value in list(context_outputs.items()):
...
def return_outputs(readable_output, outputs=None, raw_response=None, timeline=None, ignore_auto_extract=False):
    ...
    if timeline_list:
        for tl_obj in timeline_list:
            if 'Category' not in list(tl_obj.keys()):
...
def camelize(src, delim=' ', upper_camel=True):
    ...
    return {camelize_str(key): value for key, value in list(src.items())}
...
def replace_in_keys(src, existing='.', new='_'):
    ...
    return {replace_str(k): v for k, v in list(src.items())}
...
def snakify(src):
    ...
    :rtype: ``dict``
    """
    return {camel_case_to_underscore(k): v for k, v in list(src.items())}
...
'Malicious email'}, 'Address': 'user@example.com'}}
    ...
    if indicator_type_lower not in INDICATOR_TYPE_TO_CONTEXT_KEY:
        raise DemistoException('illegal indicator type, expected one of {}, got `{}`'.format(
            list(INDICATOR_TYPE_TO_CONTEXT_KEY.keys()), indicator_type_lower
        ))
    # handle files
...
, 'MD5': 'md5hash'}}
    ...
    if indicator_type_lower in INDICATOR_TYPE_TO_CONTEXT_KEY:
        ...
        raise DemistoException('Wrong indicator type supplied: {}, expected {}'
                               .format(indicator_type, list(INDICATOR_TYPE_TO_CONTEXT_KEY.keys())))
...
def merge_lists(original_list, updated_list, key):
    ...
    removed = [obj for obj in list(original_dict.values()) if obj.get('remove', False) is True]
    for r in removed:
        demisto.debug('Removing from integration context: {}'.format(str(r)))

    merged_list = [obj for obj in list(original_dict.values()) if obj.get('remove', False) is False]
...
def update_integration_context(context, object_keys=None, sync=True):
    ...
    for key, _ in list(context.items()):
...
def get_message_global_vars():
    ...
    globals_dict_full = {}
    for current_key in list(globals_dict.keys()):
    ...
    globals_sorted_by_size = sorted(list(globals_dict_full.values()), key=lambda d: d['size'], reverse=True)
...
def remove_old_incidents_ids(found_incidents_ids, current_time, look_back):
    ...
    for inc_id, addition_time in list(found_incidents_ids.items()):
